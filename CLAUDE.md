...
- Build in small chunks. After each chunk: run it, test it (curl / real Discord), then hand it to the developer to commit.
- **The AI never commits or pushes.** Make the changes, then list the changed files and suggest a commit message; the developer reviews and commits. Read-only git commands are fine.
